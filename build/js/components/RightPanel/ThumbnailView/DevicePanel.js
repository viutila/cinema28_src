...
export default class DevicePanel extends React.Component {
  ...
  /*renderNoDevice() {
    switch (this.state.type) {
      case "USB":
        if (this.state.deviceByType.usbDevs.length != 0) {
          return this.renderTypePanel();
        } else {
          return this.renderNoDeviceUSB();
        }
      case "Bluetooth":
        if (this.state.deviceByType.bluetoothDevs.length != 0) {
          return this.renderTypePanel();
        } else {
          return this.renderNoDeviceBluetooth();
        }
      case "network":
        if (this.state.deviceByType.networkDevs.length != 0) {
          return this.renderTypePanel();
        } else {
          return this.renderNoDeviceNetwork();
        }
      default: 
        return this.renderTypePanel();
    }
    ...
    let rst = deviceItemAry.map(
      (item,index)=>{
        ...
        if (index+1 == deviceNum) {
          if(deviceNum % itemNum != 0)
            a.push(self.renderEmptyStatus())
  ...
  render() {
    ...
    let settings = {
      ...
      //   { breakpoint: 1520, settings: { slidesToShow: 1, slidesToScroll: 1 } },
      // ],
      //fade: true,
    };
    ...
    return (
      <div style={divDevicePanel}>
        ...
        {/*this.renderNoDevice()*/}
        <div style={sbStyle} className="devicePanel_tv">
          <Slider {...settings}>
            {sliderDiv}
          </Slider>
        </div>
...
class CarouselArrowRight extends React.Component {
  ...
  render() {
    ...
    let arrowBtn = $('.slick-next').hasClass('slick-disabled')
...
class CarouselArrowLeft extends React.Component {
  ...
  render() {
    ...
    let arrowBtn = this.props.currentSlide === 0
      ? (<button key="arrowLeftBtnDis" style={{position:'absolute',right:'10px'}} className="arrowLeftBtnDis" />)
