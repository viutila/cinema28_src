...
export default class MsgPopupHDMI extends React.Component{
	constructor() {
	    super();
	...
	componentWillMount() {
		CommonStore.on("toggleMsgPopupHDMI",this.toggleMsgPopup);
	}

	componentWillUnmount() {
		CommonStore.removeListener("toggleMsgPopupHDMI",this.toggleMsgPopup);
	}
	...
	doYesEvent() {
		if(this.state.yEvent != null){
			...
			CommonActions.toggleMsgPopupHDMI(false);
		}
	}
    ...
    renderBtnPanel() {
    	...
    	switch(btnType) {
    		case "Both":
    			return (<div className='footer'><button className='yesBtnStyle' onClick={this.doYesEvent}>{yBtnText}</button><button className='noBtnStyle' onClick={this.doNoEvent}>{nBtnText}</button></div>);
			case "Y" :
				return (<div className="footer"><button className="yesBtnStyle" onClick={this.doChooseEvent.bind(this)}>{yBtnText}</button></div>);
			case "N" :
				...
				<Modal isOpen={this.state.isOpen} overlayClassName="OverlayForMsgPopup" className="msgPopup msgPopupHDMI" contentLabel="Modal">
					<div>
						...
						<div className="content">
                            ...
                            <p onClick={this.radioClickFunc.bind(this,'HDPlayer')} style={{marginLeft:'48px'}}>
                                <RadioButton name="HDPlayer" 
                            ...
                            <p onClick={this.radioClickFunc.bind(this,'AudioOutput')} style={{marginLeft:'48px'}}>
                                <RadioButton name="AudioOutput" 
                                    clickFunc={this.radioClickFunc.bind(this)} 
                                    currentName={this.state.pcRadio} 
                                    key={Math.random()}/>
                                &nbsp;
                                {lang.getLang('HDMI Audio output')}
                            </p>
